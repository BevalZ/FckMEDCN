...
已完成：题库扩充（35 题）；顺带修复一个真视觉 bug——医院/规培的电梯格
因字符冲突（'T' 与校园的树重复，switch 重复 case 不可达）一直被画成树，
已改名 'U' 并加 `tilemap-chars.spec.ts` 静态兜底。
剩余可选项（与用户确认后再动）：
1. **用户试玩**：回归清单的 5 条试玩路径（known-issues 末尾），顺手确认 A2 文字观感。
2. **数据对比系统**（结局页"真实数据 vs 你的数据"）：曾被移除的特性，是否恢复需确认。
3. **阶段真换图**：评估结论——各阶段贴图本就按字符定制（急诊红十字/手术室无影灯/病房等），
  "换图"本质是扩字符集与贴图细节，价值有限；建议跳过或等用户美术方向。
