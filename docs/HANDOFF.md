...
代码层面已无待办。可选方向（与用户确认后再动）：
1. **用户试玩**：回归清单的 5 条试玩路径（known-issues 末尾），顺手确认 A2 文字观感。
2. **新内容**：事件扩充（职业后期 2040+ 叙事、track 分化加深）、新 NPC、新小游戏。
3. **数值微调**：balance-sim / lifecycle-sim 的轨迹数据都在测试输出里，可按需调参。
4. **新系统**：成就/图鉴、多周目继承、难度模式——都需先与用户讨论范围。
