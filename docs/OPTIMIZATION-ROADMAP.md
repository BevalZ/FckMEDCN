...
- [x] 多周目：属性可"部分继承/重塑"（通关后开局 R 随机重塑 / I 继承上局，`attrReshape.ts`）
...
- [x] 增加"医疗损害鉴定"环节：鉴定结论加权随机（`career_lawsuit_appraisal` + `rollOutcome`/`flagAdjust`）
- [x] 私了/胜诉后增加"长期阴影/名誉"echo（`events_career_litigation.ts` 延迟回声）
- [x] 增加低概率"二审/后续追诉"阴影事件（第 12–16 季 `career_second_appeal`）

---

## Round 6 · 执业环境：职业暴露与医保监管缺失

**实据**：医保拒付/DRG/亚专科劳累/夜班已建；针刺伤与飞检链已落地；急诊第五亚专科已落地。
**现实**：针刺伤/院感暴露是真实高频；医保飞行检查/科室自查普遍；急诊/儿科高离职。

**任务**：
- [x] 新增"职业暴露"事件链：针刺伤 → 上报/预防用药 → 感染风险 → 心理阴影
- [x] 新增"医保飞检/科室自查"事件（经济+合规压力）
...
**实据**：逐项核对新批次与旧系统交互：
- 诉讼 × 亚专科：已按内/外/妇/儿/急诊分化
- 诉讼 × 职级：`rankScaled` 已用于诉讼赔付
- 导师风格 × 家境：`balance-matrix.spec.ts` 覆盖拮据+抠门极端组合
- 资产 × 结局：应急提现已有；结局财务卡区分现金/资产/负债
- HUD × 新字段：职业期科室+医院档位+编制/合同；硕博导师风格

**任务**：
- [x] 诉讼按亚专科分化（含急诊）与职级缩放
