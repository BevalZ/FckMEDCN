...
> 现状基线：全量 90 用例绿，`npx tsc --noEmit` + `npm run build` + `npx playwright test`。
...
**任务**：
- [ ] 职业期 MAX_TURNS 12→16~20，给"熬年限"留空间；晋升事件加"任职年限"门槛（需在职业期满 N 季）
- [ ] 增加"住院总医师"一年期固定剧情（体力/心理双压榨 + 结业奖）
- [ ] 晋升增加失败分支（材料被退/差一分/名额被占），当前只有一次机会太顺
- [ ] 结局页真实数据卡按职称/地区刷新（年薪制口径：主任 30w/副主任 25w/主治 20w；东部三甲 40-60w）
...
**任务**：
- [ ] 属性成长：增加"运气可被经历改变"（重大挫折→运气微降/救助→升）、"外貌随年龄/职业磨损"（长期夜班→外貌降）事件
- [ ] 资产提现/用途：增加"购房首付用资产抵扣""子女教育基金""提前还贷"等消耗口（与 Round 2 房价联动）
...
**任务**：
- [ ] 诉讼结果接入"病历质量"：`career_fin_record_fine` 若发生过病历扣费，诉讼更被动（证据瑕疵）
...
- 资产 × 结局：资产计入负债判定但无"提现"路径，玩家无法用资产应急——**资产是"看得见摸不着"的钱**
- HUD × 新字段：HUD 显示 attrs/家庭/理财/资产，但**不显示亚专科、导师风格、医院/地区档位**

**任务**：
- [ ] 诉讼按亚专科分化（外科手术纠纷/儿科诊疗纠纷/产科医疗事故/内科漏诊误诊）与职级缩放
- [ ] balance-sim 增加"拮据+抠门导师"极端组合断言，防过苛
- [ ] 资产增加"应急提现"入口（R 菜单或事件）
...
**实据**：90 用例按主题分布——经济(6)/属性(5)/事件权重(1)/新闻(2)/技能中心(2)/性别(7)/图鉴徽章传承(3)/导航(10+)/ESC(3)/sim(2)/其他。
**缺口**：
- 无"死 flag 回响"回归（Round 1 的 114 个漏洞没有测试拦截）
- 无"地区/医院薪资房价差异化"回归（Round 2 尚未实现）
- sim 只有造假 vs 诚实两流，无"临床轨 vs 科研轨 vs 不同地区"矩阵
- 强制事件（诉讼/亚专科）只测了"可达"，未测"ESC 跳过仍补触发"的完整路径

**任务**：
- [ ] `flag-echo-coverage.spec.ts`（Round 1 的回归）
- [ ] `region-economy.spec.ts`（地区薪资/房价差异化回归）
- [ ] balance-sim 扩展：临床轨/科研轨/一线三甲/县城基层 各跑一次，断言最终存款与职称分布
- [ ] career-nodes 补"ESC 跳过诉讼 → 下季仍强制"用例
...
| 优先级 | 项目 | 轮 | 理由 |
|---|---|---|---|
| **P0** | 地区/医院薪资与房价差异化 | 2 | 用户提问；人生最大经济分岔当前零影响 |
| **P0** | 死 flag 回响补全 + 回归 | 1 | 114 个选择无后果，"选择要回响"原则大面积失效 |
| P1 | 诉讼按亚专科/职级缩放 + 证据反哺 | 5/9 | 让已建诉讼系统有深度与交叉 |
| P1 | 晋升失败分支 + 年限门槛 + 职业期延长 | 3 | 核心职业体验"熬" |
| P1 | 职业暴露 / 医保飞检 | 6 | 真实高频且教育价值高 |
| P1 | 资产提现 + 属性成长 | 4/9 | 让现有系统闭环 |
| P2 | 一作之争 / 临床型晋升 | 7 | 呼应破五唯 |
| P2 | 值班错过家庭 / 子女叙事 | 8 | 共鸣向粘性 |
| P2 | 极端组合 balance 断言 / 强制事件回归 | 9/10 | 测试防线 |
| P3 | 亚专科第 5 项(急诊)/时间轴重构 | 3/6 | 结构改造，风险大 |
