...
- [x] 职业期两轮诉讼按内/外/妇/儿四科分化，赔付按职级缩放，ESC 补触发有回归
...
2. 将 QuestLog 从静态文本改为 NPC/地点指向，完成任务后给出好感、徽章或新闻反馈。
3. 为至少一个 NPC 建立“初识→误会→修复→信任”的多阶段关系链，并为社会遭遇增加有限的再遇机制。
4. 写 `docs/EVENT-LAYERING.md`：确定性人生节点、好感度互动事件、日常生成事件分别规定触发层级和权重。

验收：五个核心 NPC 的信任/疏远门控均有测试；任务完成反馈可观察；跨阶段回响不绕过 flag 门控；
事件分层规范与实际模块各有一条交叉检查。
...
1. 完善资产/HUD 闪色、理财 toast、季度结构化账单；在 H 帮助中列明各场景 ESC 语义，并补 Q/E
   首次引导。移动端小游戏触控暂不纳入本批，除非另立需求。
2. 消除 Playwright 冷启动 flaky：避免复用陈旧 dev server，增加两次连续全量回归记录，目标为
   0 hard failure、flaky ≤ 1 且有明确环境说明。
3. 更新路线图、审查报告、HANDOFF，核对旧快照不再误导；确认未提交用户文档变更未被覆盖。

### 暂缓项

“急诊”第五亚专科/轮转机制、完整移动端触控、SEO/分享卡片和大规模结局重构暂缓到 M9–M13 完成后；
先用平衡矩阵和真实回归证明现有四个亚专科、经济和事件层稳定，再进行结构性扩张。
