...
export class GuipeiWalkScene extends Phaser.Scene {
  ...
  private refreshInfoBar() {
    ...
    this.infoLabel.setText(
      `第${s.year}年 Q${s.quarter} | ${s.stats.age}岁 | 规培第 ${days}/${MAX_TURNS} 季  ·  移动 WASD/方向键 · 交互 E · 任务 Q · R 游戏菜单`,
    );
    ...
    const doneHints = this.questLog?.setItems(guipeiQuests(s.flags, this.actionsLeft, this.storyletUsed)) ?? [];
  ...
  private placeNpcs() {
    ...
    for (const def of npcsForStage(STAGE as LifeStage)) {
      ...
      const sprite = new NpcSprite(this, def, c.x, c.y, spotId, {
        anchorCol: tile.col,
        anchorRow: tile.row,
        cols: GUIPEI_SPEC.cols,
        rows: GUIPEI_SPEC.rows,
  ...
  private transitionToNext() {
    ...
    this.cameras.main.once('camerafadeoutcomplete', () => {
      ...
      const next = nextSceneAfterGuipei('walk');
